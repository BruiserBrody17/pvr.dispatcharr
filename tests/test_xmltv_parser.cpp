...
<tv>
  <programme channel="2632" start="20260101120000 +0000" stop="20260101130000 +0000">
...
TEST_CASE("Parses a fully-populated programme entry", "[XmlTvParser]")
{
  ...
  CHECK(error.empty());
  REQUIRE(out.size() == 1);
  REQUIRE(out.count("2632") == 1);
  REQUIRE(out.at("2632").size() == 1);

  const EpgEntry& entry = out.at("2632")[0];
  CHECK(entry.channelXmltvId == "2632");
